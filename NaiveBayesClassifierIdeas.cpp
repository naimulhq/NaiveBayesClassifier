...
// This file includes all the ideas I tried to implement but did not end up in final version due to its inability
...
double getEntropies(double,double,double,double,double,double);
    ...
    for(int i = 0; i < featureLength; i++){
        ...
        if(ignore){
            ...
            if(features[i].compare("age") != 0){
                ...
                double prob_true_given_survive = (double(yes_Survive) + 1)/((counts_survive) + 2);
                double prob_false_given_survive = (double(no_Survive) + 1)/((counts_survive) + 2);
                double prob_true_given_not_survive = (double(yes_NoSurvive) + 1)/((counts_dead) + 2);
                double prob_false_given_not_survive = (double(no_NoSurvive) + 1)/((counts_dead) + 2);

                double entropy = getEntropies(yes_Survive,no_Survive, yes_NoSurvive,no_NoSurvive, counts_survive, counts_dead);
                cout << "Entropy for " << features[i] << ": " << entropy << endl;
...
double getEntropies(double yes_Survive, double no_Survive, double yes_NoSurvive, double no_NoSurvive, double counts_survive, double counts_dead){
    double part1 = double(yes_Survive + yes_NoSurvive)/(counts_survive + counts_dead);
    double part2 = double(no_Survive + no_NoSurvive)/(counts_survive + counts_dead);

    double part3 = double(yes_Survive)/(yes_Survive + yes_NoSurvive);
    double part4 = double(yes_NoSurvive)/(yes_Survive + yes_NoSurvive);

    double part5 = double(no_Survive)/(no_Survive+no_NoSurvive);
    double part6 = double(no_NoSurvive)/(no_NoSurvive + no_Survive);

    double part7 = -1 * ((part3*log(part3)) + (part4 * log(part4)));
    double part8 = -1 * ((part5*log(part5)) + (part6 * log(part6)));
    return part1*part7 + part2*part8;
}
